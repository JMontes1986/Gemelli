...
contract GemelliAuditLog {
     ...
     * @param _newOwner Nueva dirección propietaria
     */
    function transferOwnership(address _newOwner) external onlyOwner {
        require(_newOwner != address(0), "Nueva direccion invalida");
        require(_newOwner != owner, "Ya eres el propietario");
        
        address previousOwner = owner;
        
        // Remover autorización del owner anterior si no está en la lista
        if (!isAuthorized[previousOwner]) {
            isAuthorized[previousOwner] = false;
        }
        
        // Autorizar al nuevo owner
        if (!isAuthorized[_newOwner]) {
            isAuthorized[_newOwner] = true;
            authorizedAddresses.push(_newOwner);
        }
    ...
    /**
     * @dev Convierte un string a bytes32
     * @param _source String a convertir
     * @return result bytes32 resultante
     */
     ...
     * @param _bytes32 bytes32 a convertir
